...
import net.renfei.validator.annotation.preset.LengthMaxCheck;
 ...
 * @author renfei
 */
public class LengthMaxVerifierImpl implements Verifier {
    ...
    public VerifiedReportDetail verification(Annotation annotation, Object object, Field field) {
        ...
        LengthMaxCheck lengthMaxCheck = (LengthMaxCheck) annotation;
        try {
            if (field.get(object) == null) {
                report.setPassed(false);
            } else {
                if (field.get(object).getClass().isArray()) {
                    report.setPassed(((Object[]) field.get(object)).length <= lengthMaxCheck.max());
                } else if (field.get(object) instanceof String) {
                    report.setPassed(field.get(object).toString().length() <= lengthMaxCheck.max());
                } else if (field.get(object) instanceof List) {
                    report.setPassed(((List<?>) field.get(object)).size() <= lengthMaxCheck.max());
                } else if (field.get(object) instanceof Map) {
                    report.setPassed(((Map<?, ?>) field.get(object)).size() <= lengthMaxCheck.max());
                } else if (field.get(object) instanceof AtomicIntegerArray) {
                    report.setPassed(((AtomicIntegerArray) field.get(object)).length() <= lengthMaxCheck.max());
                } else if (field.get(object) instanceof AtomicLongArray) {
                    report.setPassed(((AtomicLongArray) field.get(object)).length() <= lengthMaxCheck.max());
                } else if (field.get(object) instanceof AtomicReferenceArray) {
                    report.setPassed(((AtomicReferenceArray<?>) field.get(object)).length() <= lengthMaxCheck.max());
                } else {
        ...
        report.setBusinessName(lengthMaxCheck.businessName());
        return report;
    }
}
