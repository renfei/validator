...
import net.renfei.validator.annotation.preset.LengthCheck;
 ...
 * @author renfei
 */
public class LengthVerifierImpl implements Verifier {
    ...
    public VerifiedReportDetail verification(Annotation annotation, Object object, Field field) {
        ...
        LengthCheck lengthCheck = (LengthCheck) annotation;
        try {
            if (field.get(object) == null) {
                report.setPassed(false);
            } else {
                if (field.get(object).getClass().isArray()) {
                    report.setPassed(((Object[]) field.get(object)).length == lengthCheck.length());
                } else if (field.get(object) instanceof String) {
                    report.setPassed(field.get(object).toString().length() == lengthCheck.length());
                } else if (field.get(object) instanceof List) {
                    report.setPassed(((List<?>) field.get(object)).size() == lengthCheck.length());
                } else if (field.get(object) instanceof Map) {
                    report.setPassed(((Map<?, ?>) field.get(object)).size() == lengthCheck.length());
                } else if (field.get(object) instanceof AtomicIntegerArray) {
                    report.setPassed(((AtomicIntegerArray) field.get(object)).length() == lengthCheck.length());
                } else if (field.get(object) instanceof AtomicLongArray) {
                    report.setPassed(((AtomicLongArray) field.get(object)).length() == lengthCheck.length());
                } else if (field.get(object) instanceof AtomicReferenceArray) {
                    report.setPassed(((AtomicReferenceArray<?>) field.get(object)).length() == lengthCheck.length());
                } else {
        ...
        report.setBusinessName(lengthCheck.businessName());
        return report;
    }
}
